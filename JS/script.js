function init (){
    ...
    gsap.from(logos, { duration: 2, opacity: 0, y: -50, ease: "bounce.out" });

    let categories = document.querySelectorAll("h4");
    categories.forEach(focus => {
        gsap.from(focus, { duration: 1, opacity: 0, y: 50, stagger: 0.3, ease: "power2.out" });
    });       
};
